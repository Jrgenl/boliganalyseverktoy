"""
Streamlit web app for boliganalyseverktøy - Web Version

Dette scriptet er en tilpasset versjon av app.py for web-distribusjon.
"""
...
def analyze_property(url):
    ...
    try:
        ...
        with st.spinner('Henter boligdata fra Finn.no...'):
            raw_data = scraper.get_bolig_data(url)
        ...
        if "error" in raw_data:
            st.error(f"Feil ved henting av data: {raw_data['error']}")
            return None
        ...
        # Lagre analyserte data i session state
        if "analyzed_properties" not in st.session_state:
            st.session_state.analyzed_properties = {}
        
        st.session_state.analyzed_properties[url] = {
...
# Funksjon for å vise historikk
def display_history():
    """
    Viser historikk over tidligere analyserte boliger.
    """
    if "analyzed_properties" not in st.session_state or not st.session_state.analyzed_properties:
        st.info("Ingen tidligere analyser funnet.")
        return
    
    st.header("Tidligere analyserte boliger")
    
    # Sorter etter tidspunkt (nyeste først)
    sorted_urls = sorted(
        st.session_state.analyzed_properties.keys(),
        key=lambda url: st.session_state.analyzed_properties[url]['timestamp'],
        reverse=True
    )
    
    for i, url in enumerate(sorted_urls[:5]):  # Vis bare de 5 siste
        property_data = st.session_state.analyzed_properties[url]
        bolig_data = property_data['bolig_data']
        
        with st.expander(f"{i+1}. {bolig_data.tittel or bolig_data.adresse or url}"):
            st.markdown(f"**URL:** {url}")
            st.markdown(f"**Adresse:** {bolig_data.adresse}")
            if bolig_data.prisantydning > 0:
                st.markdown(f"**Prisantydning:** {bolig_data.prisantydning:,} kr".replace(',', ' '))
            st.markdown(f"**Boligtype:** {bolig_data.boligtype}")
            
            if st.button(f"Analyser på nytt", key=f"reanalyze_{i}"):
                st.session_state.finn_url = url
...
def main():
    ...
    if "finn_url" not in st.session_state:
        st.session_state.finn_url = ""
    ...
    finn_url = st.sidebar.text_input("Lim inn Finn.no boligannonse URL:", value=st.session_state.finn_url)
    ...
    **Kontakt**
    ...
    [Send e-post](mailto:kontakt@boliganalyse.no)
    """)
    
    # Hovedinnhold
    if analyze_button or finn_url != st.session_state.finn_url:
    ...
    <div style="text-align: center; color: #666;">
        <p>Boliganalyseverktøy © 2025 | Utviklet av Manus</p>
    </div>
    """, unsafe_allow_html=True)
